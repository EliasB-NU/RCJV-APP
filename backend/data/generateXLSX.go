...
func GenerateXLSX(league string, db *gorm.DB) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("Error finding fields: %v", err)
	}

	var teams []database.Team
	err = db.Where("league = ?", league).Find(&teams).Error
	if err != nil {
		return "", fmt.Errorf("Error finding teams: %v", err)
	}
	...
	for index, field := range fields {
		...
		if err != nil {
			return "", fmt.Errorf("Error setting cell value (field): %v\n", err)
		}
	}
	...
	for index, team := range teams {
		...
		if err != nil {
			return "", fmt.Errorf("Error setting cell value (team): %v\n", err)
		}
	}
